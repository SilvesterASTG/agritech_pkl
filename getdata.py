...
def update_and_download_data(sensor_company_id, metric, days):
    token = get_access_token()
    headers = {"Authorization": f"Bearer {token}"}
    today = datetime.today().date()
    start_date = f"{today} 00:00:00"
    end_date = f"{(today + timedelta(days=days))} 23:59:59"
...
def get_sensor_data(station_name, metric, days):
    station_mapping = {
       "OP1": {
    ...
    if sensor_company_id:
        return update_and_download_data(sensor_company_id, metric, days)
    return 0, 0, 0
...
def main():
    ...
    for device_name, station_name, jenis_a, tanggalupdate in devices:
        ...
        max_airhum, min_airhum, avg_airhum = get_sensor_data(station_name, 'airhum', days)
        max_airtemp, min_airtemp, avg_airtemp = get_sensor_data(station_name, 'airtemp', days)
        max_airpress, min_airpress, avg_airpress = get_sensor_data(station_name, 'airpress', days)
        _, _, avg_ws = get_sensor_data(station_name, 'windspeed', days)
